        ...
        function login() {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response == "OK") {
                        localStorage.show_welcomeMessage = 0;
                        location.href = "main";
...
if (subMenu == "result") {
    $("#welcomeMessage").click(function() {
        localStorage.show_welcomeMessage = 1;
        $(".welcomeMessage").parent().remove();
    });
    function setPercent() {
        $("div[class='progress-bar-circle progress-bar-banner position-relative']").each(function(i, e) {
            if (i == 0) {
                $(this).find("div").append("<br/><span style='font-size: 14px;'>" + overall + "%</span>");
            } else if (i == 1) {
                $(this).find("div").append("<br/><span style='font-size: 14px;'>" + gdpr + "%</span>");
            }
        });
    }
    document.addEventListener("DOMContentLoaded", function(event) {
        if (localStorage.show_welcomeMessage == 1) {
            $(".welcomeMessage").parent().remove();
        }
    });
...
if (subMenu == "authority_report") {
    $(".exportAuthority").click(function() {
        if (confirm("Are you sure export as a PDF file?")) {
            window.open(site_url + "document/authorityExport", "_blank");
        }
    });
}
if (subMenu == "management_report") {
    $(".export").click(function() {
        if (confirm("Are you sure export as a PDF file?")) {
            window.open(site_url + "document/managementExport", "_blank");
        }
    });
}